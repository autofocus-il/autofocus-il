...
class DiffusionPolicyConfig(BaseConfig):
    ...
    def algo_config(self):
        ...
        self.algo.optim_params.policy.learning_rate.num_cycles = (
            0.5  # number of cosine cycles (used by "cosine" scheduler)
        )
        self.algo.optim_params.policy.learning_rate.warmup_steps = (
            500  # number of warmup steps (used by "cosine" scheduler)
        )
        self.algo.optim_params.policy.learning_rate.epoch_schedule = (
            []
        )  # epochs where LR decay occurs (used by "linear" and "multistep" schedulers)
