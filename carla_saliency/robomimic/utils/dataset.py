...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # When using cache_mode "all", setting this to True will additionally
        # precompute and store the outputs of all get_item calls. This is fast
        # for indexing but can explode memory usage due to duplication.
        # Default is False to favor memory usage.
        hdf5_cache_getitem=False,
        ...
        self.hdf5_cache_getitem = hdf5_cache_getitem
        ...
        self.load_demo_info(
            filter_by_attribute=self.filter_by_attribute, demo_limit=demo_limit
        )
        ...
        if self.hdf5_cache_mode in ["all", "low_dim"]:
            ...
            # Optionally cache get_item calls in "all" mode. This can be very
            # memory intensive due to duplicate slices for overlapping sequences.
            if self.hdf5_cache_mode == "all" and self.hdf5_cache_getitem:
                if _is_rank0():
                    print("SequenceDataset: caching get_item calls...")
                self.getitem_cache = [
                    self.get_item(i) for i in LogUtils.custom_tqdm(range(len(self)))
                ]
                # In this mode, drop the raw cache to free memory.
                del self.hdf5_cache
    ...
    def normalize_obs(self):
        ...
        for k in merged_stats:
            ...
            obs_normalization_stats[k]["offset"] = merged_stats[k]["mean"].astype(
                np.float32
            )
    ...
    def get_action_traj(self, ep):
        ...
        for key in self.action_keys:
            ...
            if (
                self.hdf5_cache is not None
                and ep in self.hdf5_cache
                and key in self.hdf5_cache[ep]
            ):
                # Avoid unnecessary copies: cached arrays are already float32 from load_dataset_in_memory
                arr = self.hdf5_cache[ep][key]
                if arr.dtype != np.float32:
                    # Cast without forcing a copy when possible
                    arr = arr.astype(np.float32, copy=False)
                action_traj[key] = arr
            else:
                action_traj[key] = self.hdf5_file["data/{}/{}".format(ep, key)][
                    ()
                ].astype("float32")
        return action_traj
    ...
    def get_action_normalization_stats(self):
        ...
        """
        # If not yet computed, decide whether to fast-path (no normalization)
        # or compute full statistics depending on action_config.
        if self.action_normalization_stats is None:
            # Check if all action keys specify no normalization
            no_norm = True
            for k in self.action_keys:
                if self.action_config.get(k, {}).get("normalization", None) is not None:
                    no_norm = False
                    break

            if no_norm:
                # Build identity normalization stats without scanning the dataset
                identity_stats = OrderedDict()
                for k in self.action_keys:
                    # Determine shape for this action key: prefer cache if available
                    shape = None
                    if self.hdf5_cache is not None:
                        # Use first demo present in cache
                        ep0 = self.demos[0]
                        if ep0 in self.hdf5_cache and k in self.hdf5_cache[ep0]:
                            shape = self.hdf5_cache[ep0][k].shape[1:]
                    if shape is None:
                        # Fall back to file shape
                        with self.hdf5_file_opened() as f:
                            ds = f["data/{}/{}".format(self.demos[0], k)]
                            shape = ds.shape[1:]
                    # Ensure at least 1D
                    if len(shape) == 0:
                        shape = (1,)
                    scale = np.ones((1,) + tuple(shape), dtype=np.float32)
                    offset = np.zeros((1,) + tuple(shape), dtype=np.float32)
                    identity_stats[k] = {"scale": scale, "offset": offset}
                self.action_normalization_stats = identity_stats
            else:
                # Full pass over dataset to compute stats
    ...
    def __getitem__(self, index):
        ...
        """
        if self.hdf5_cache_mode == "all" and hasattr(self, "getitem_cache"):
...
class MetaDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # If no normalization requested, skip expensive dataset scans
        action_config = self.datasets[0].action_config
        no_norm = True
        for k in action_config.keys():
            if action_config.get(k, {}).get("normalization", None) is not None:
                no_norm = False
                break
        if no_norm:
            # Reuse the single-dataset fast path to compute identity stats
            ident = self.datasets[0].get_action_normalization_stats()
            self.set_action_normalization_stats(ident)
        else:
            action_stats = self.get_action_stats()
            self.action_normalization_stats = action_stats_to_normalization_stats(
