    ...
    for sensor in sensors:
        ...
        if agent_track == Track.SENSORS:
            if sensor["type"].startswith("sensor.opendrive_map"):
                raise SensorConfigurationInvalid(
                    "Illegal sensor 'sensor.opendrive_map' used for Track [{}]!".format(
                        agent_track
                    )
                )
        ...
        if sensor["type"] not in ALLOWED_SENSORS:
            raise SensorConfigurationInvalid(
                "Illegal sensor '{}' used for Track [{}]!".format(
                    sensor["type"], agent_track
                )
            )
...
class AgentWrapper(object):
    ...
    def setup_sensors(self, vehicle):
        ...
        for sensor_spec in self._agent.sensors():
            type_, id_, sensor_transform, attributes = self._preprocess_sensor_spec(
                sensor_spec
            )
...
class ROSAgentWrapper(AgentWrapper):
    ...
    def _preprocess_sensor_spec(self, sensor_spec):
        type_, id_, sensor_transform, attributes = super(
            ROSAgentWrapper, self
        )._preprocess_sensor_spec(sensor_spec)
    ...
    def setup_sensors(self, vehicle):
        ...
        """
        for sensor_spec in self._agent.sensors():
            type_, id_, transform, attributes = self._preprocess_sensor_spec(
                sensor_spec
            )
            uid = self._agent.spawn_object(
                type_, id_, transform, attributes, attach_to=vehicle.id
            )
