...
class HumanInterface(object):
    ...
    def run_interface(self, input_data):
        ...
        image_center = cv2.resize(
            image_center, (self.display_width, self.display_height)
        )
        ...
        if self._left_mirror:
            ...
            image_left = cv2.resize(
                image_left,
                (
                    int(self.display_width * self._scale),
                    int(self.display_height * self._scale),
                ),
            )
            left_surface = pygame.surfarray.make_surface(image_left.swapaxes(0, 1))
            self._surface.blit(
                left_surface, (0, (1 - self._scale) * self.display_height)
            )
        ...
        if self._right_mirror:
            ...
            self._surface.blit(
                right_surface,
                (
                    (1 - self._scale) * self.display_width,
                    (1 - self._scale) * self.display_height,
                ),
            )
...
class HumanAgent(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        self.camera_width = 640 * 1.5
        self.camera_width_ = 1920
        self.camera_height = 360 * 1.5
        self.camera_height_ = 1080
