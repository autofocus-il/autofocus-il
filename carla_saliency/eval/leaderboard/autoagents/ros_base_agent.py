...
class ROSLogger(object):
    ...
    def __init__(self, name):
        ...
        self.handler = logging.handlers.RotatingFileHandler(
            os.path.join(logger_path, self.name + ".log"),
...
class ROSLauncher(object):
    ...
    def run(self, package, launch_file, parameters={}, wait=False):
        ...
        self._process = pexpect.spawn(
            cmdline, encoding="utf-8", logfile=self._logger if self.debug else None
        )
    ...
    def carla2ros_pose(cls, x, y, z, roll, pitch, yaw, to_quat=False):
        ...
        if to_quat:
            out_orientation = cls.rpy2quat(
                out_orientation["roll"],
                out_orientation["pitch"],
                out_orientation["yaw"],
            )
...
class ROSBaseAgent(AutonomousAgent):
    ...
    def __init__(self, ros_version, carla_host, carla_port, debug=False):
        ...
        self._bridge_process = ROSLauncher(
            "bridge", ros_version=ros_version, debug=debug
        )
        self._bridge_process.run(
            ...
            launch_file=(
                ...
                if ros_version == 1
                else "carla_ros_bridge.launch.py"
            ),
            parameters={
                "host": carla_host,
                "port": carla_port,
    ...
    def _vehicle_control_cmd_callback(self, control_msg):
        if isinstance(control_msg, dict):
            control_timestamp = (
                control_msg["header"]["stamp"]["secs"]
                + control_msg["header"]["stamp"]["nsecs"] * 1e-9
            )
            ...
            control_timestamp = (
                control_msg.header.stamp.sec + control_msg.header.stamp.nanosec * 1e-9
            )
            control = carla.VehicleControl(
                steer=control_msg.steer,
                throttle=control_msg.throttle,
        ...
        if (
            self._last_control_timestamp is not None
            and abs(self._last_control_timestamp - control_timestamp) < EPSILON
        ):
            print(
                "\033[93mWARNING: A new vehicle command with a repeated timestamp has been received {} .\033[0m".format(
                    control_timestamp
                ),
        ...
        if abs(control_timestamp - carla_timestamp) > EPSILON:
            print(
                "\033[93mWARNING: Expecting a vehicle command with timestamp {} but the timestamp received was {} .\033[0m".format(
                    carla_timestamp, control_timestamp
                ),
                "\033[93mThis vehicle command will be ignored.\033[0m",
                sep=" ",
            )
