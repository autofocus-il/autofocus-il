...
class RouteIndexer:
    ...
    def validate_and_resume(self, endpoint):
        ...
        if progress[1] != self.total:
            print(
                "Problem reading checkpoint. Endpoint's amount of routes does not match the given one"
            )
            return False
        ...
        while check_index < resume_index:
            try:
                ...
                route_id += "_rep" + str(
                    self._configs_list[check_index].repetition_index
                )
                checkpoint_route_id = route_data[check_index]["route_id"]

                if route_id != checkpoint_route_id:
                    print(
                        "Problem reading checkpoint. Checkpoint routes don't match the current ones"
                    )
                    return False
        ...
        if entry_status == "Crashed":
            self.index = max(
                0, resume_index - 1
            )  # Something went wrong, repeat the last route
        else:
