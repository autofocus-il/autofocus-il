...
class Checkpoint:
    ...
    def to_json(self):
        ...
        d["records"] = []
        d["records"] = [
            x.to_json() for x in self.records if x.index != -1
        ]  # Index -1 = Route in progress
...
class StatisticsManager(object):
    ...
    def clear_records(self):
        ...
        if not self._endpoint.startswith(("http:", "https:", "ftp:")):
            ...
            os.makedirs(os.path.dirname(self._endpoint), exist_ok=True)
        ...
        with open(self._debug_endpoint, "w") as f:
            ...
            for e in all_events[:5]:
                ...
                if event_type in PENALTY_VALUE_DICT:
                    string += (
                        " (penalty: " + str(PENALTY_VALUE_DICT[event_type]) + ")\n"
                    )
                elif event_type in PENALTY_PERC_DICT:
                    string += (
                        " (value: " + str(round(e.get_dict()["percentage"], 3)) + "%)\n"
                    )
    ...
    def compute_route_statistics(
        ...
        route_record.scores["score_composed"] = round(
            max(score_route * score_penalty, 0.0), ROUND_DIGITS_SCORE
        )

        # Update result
        route_record.num_infractions = sum(
            [len(route_record.infractions[key]) for key in route_record.infractions]
        )

        if target_reached:
            route_record.status = (
                "Completed" if route_record.num_infractions > 0 else "Perfect"
            )
        else:
    ...
    def compute_global_statistics(self):
        ...
        def get_infractions_value(route_record, key):
            ...
            if (
                key
                == PENALTY_NAME_DICT[TrafficEventType.OUTSIDE_ROUTE_LANES_INFRACTION]
            ):
        ...
        for route_record in route_records:
            ...
            global_record.scores_mean["score_route"] += (
                route_record.scores["score_route"] / self._total_routes
            )
            global_record.scores_mean["score_penalty"] += (
                route_record.scores["score_penalty"] / self._total_routes
            )
            global_record.scores_mean["score_composed"] += (
                route_record.scores["score_composed"] / self._total_routes
            )
            ...
            global_record.meta["duration_system"] += route_record.meta[
                "duration_system"
            ]

            # Downgrade the global result if need be ('Perfect' -> 'Completed' -> 'Failed'), and record the failed routes
            route_result = (
                "Failed" if "Failed" in route_record.status else route_record.status
            )
        ...
        for item in global_record.scores_mean:
            global_record.scores_mean[item] = round(
                global_record.scores_mean[item], ROUND_DIGITS_SCORE
            )
        ...
        if self._total_routes == 1:
            ...
            for key in global_record.scores_std_dev:
                value = round(
                    math.sqrt(
                        global_record.scores_std_dev[key]
                        / float(self._total_routes - 1)
                    ),
                    ROUND_DIGITS,
                )
        ...
        for route_record in route_records:
            ...
            for key in global_record.infractions:
                global_record.infractions[key] += get_infractions_value(
                    route_record, key
                )
        ...
        for key in global_record.infractions:
            ...
            global_record.infractions[key] = round(
                global_record.infractions[key], ROUND_DIGITS
            )
        ...
        self._results.values = [
            ...
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.COLLISION_PEDESTRIAN]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.COLLISION_VEHICLE]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.COLLISION_STATIC]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.TRAFFIC_LIGHT_INFRACTION]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.STOP_INFRACTION]
                ]
            ),
            str(
                global_record.infractions[
            ...
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.ROUTE_DEVIATION]
                ]
            ),
            str(global_record.infractions["route_timeout"]),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.VEHICLE_BLOCKED]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.YIELD_TO_EMERGENCY_VEHICLE]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.SCENARIO_TIMEOUT]
                ]
            ),
            str(
                global_record.infractions[
                    PENALTY_NAME_DICT[TrafficEventType.MIN_SPEED_INFRACTION]
                ]
            ),
        ]
    ...
    def validate_and_write_statistics(self, sensors_initialized, crashed):
        ...
        if sensors_initialized and not self._results.sensors:
            ...
            elif not crashed and (
                progress[0] != progress[1] or progress[0] != len(route_records)
            ):
