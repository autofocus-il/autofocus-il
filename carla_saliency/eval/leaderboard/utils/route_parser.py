...
class RouteParser(object):
        ...
        def get_routes_subset():
            ...
            for group in subset_groups:
                if "-" in group:
                    ...
                    for route in tree.iter("route"):
                        ...
                        if not found_end and found_start:
                            if route_id in subset_ids:
                                raise ValueError(
                                    f"Found a repeated route with id '{route_id}'"
                                )
                            else:
                    ...
                    if not found_start:
                        raise ValueError(
                            f"Couldn't find the route with id '{start}' inside the given routes file"
                        )
                    if not found_end:
                        raise ValueError(
                            f"Couldn't find the route with id '{end}' inside the given routes file"
                        )
                    ...
                    if not found:
                        raise ValueError(
                            f"Couldn't find the route with id '{group}' inside the given routes file"
                        )
            ...
            for scenario in route.find("scenarios").iter("scenario"):
                ...
                for elem in list(scenario):
                    if elem.tag == "trigger_point":
                        scenario_config.trigger_points.append(
                            convert_elem_to_transform(elem)
                        )
    ...
    def parse_weather(route):
        ...
        for weather_elem in weathers_elem.iter("weather"):
            ...
            weather = carla.WeatherParameters(
                sun_altitude_angle=70, cloudiness=50
            )  # Base weather
            for weather_attrib in weather_elem.attrib:
                if hasattr(weather, weather_attrib):
                    setattr(
                        weather,
                        weather_attrib,
                        float(weather_elem.attrib[weather_attrib]),
                    )
                elif weather_attrib != "route_percentage":
                    print(
                        f"WARNING: Ignoring '{weather_attrib}', as it isn't a weather parameter"
                    )
    ...
    def is_scenario_at_route(trigger_transform, route):
        ...
        def is_trigger_close(trigger_transform, route_transform):
            ...
            dyaw = (
                float(trigger_transform.rotation.yaw) - route_transform.rotation.yaw
            ) % 360
