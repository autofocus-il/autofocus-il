...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        if debug_mode > 0:
            self._draw_waypoints(
                self.route, vertical_shift=0.1, size=0.1, downsample=10
            )
    ...
    def _filter_scenarios(self, scenario_configs):
        ...
        for scenario_number, scenario_config in enumerate(scenario_configs):
            ...
            if not RouteParser.is_scenario_at_route(trigger_point, self.route):
                print(
                    "WARNING: Ignoring scenario '{}' as it is too far from the route".format(
                        scenario_config.name
                    )
                )
                continue

            scenario_config.route_var_name = "ScenarioRouteNumber{}".format(
                scenario_number
            )
        ...
        for slot in available_parking_locations:
            slot_transform = carla.Transform(
                location=carla.Location(
                    slot["location"][0], slot["location"][1], slot["location"][2]
                ),
            ...
            in_area = (min_x < slot_transform.location.x < max_x) and (
                min_y < slot_transform.location.y < max_y
            )
    ...
    def spawn_parked_vehicles(self, ego_vehicle, max_scenario_distance=10):
        ...
        def is_close(slot_location, ego_location):
            return (
                slot_location.distance(ego_location)
                < self.PARKED_VEHICLES_INIT_THRESHOLD
            )
        ...
        for slot in self.available_parking_locations:
            ...
            if is_close(slot_transform.location, ego_location) and is_free(
                slot_transform.location
            ):
                mesh_bp = (
                    CarlaDataProvider.get_world()
                    .get_blueprint_library()
                    .filter("static.prop.mesh")[0]
                )
                mesh_bp.set_attribute("mesh_path", slot["mesh"])
                mesh_bp.set_attribute("scale", "0.9")
                new_parked_vehicles.append(
                    carla.command.SpawnActor(mesh_bp, slot_transform)
                )
                self.available_parking_locations.remove(slot)

        # Add the actors to _parked_ids
        for response in CarlaDataProvider.get_client().apply_batch_sync(
            new_parked_vehicles
        ):
    ...
    def _draw_waypoints(self, waypoints, vertical_shift, size, downsample=1):
        ...
        for i, w in enumerate(waypoints):
            ...
            self.world.debug.draw_point(
                wp, size=size, color=color, life_time=self.timeout
            )
    ...
    def build_scenarios(self, ego_vehicle, debug=False):
        ...
        if self.ego_data is None:
            self.ego_data = ActorConfigurationData(
        ...
        for scenario_config in self.missing_scenario_configurations:
            ...
            try:
                ...
                if trigger_location.distance(ego_location) < self.INIT_THRESHOLD:
                    ...
                    self.occupied_parking_locations.extend(
                        scenario_instance.get_parking_slots()
                    )

                    if debug:
                        scenario_loc = scenario_config.trigger_points[0].location
                        debug_loc = self.map.get_waypoint(
                            scenario_loc
                ...
                print(
                    f"\033[93mSkipping scenario '{scenario_config.name}' due to setup error: {e}"
                )
                if debug:
    ...
    def _create_behavior(self):
        ...
        scenario_trigger_distance = (
            DIST_THRESHOLD  # Max trigger distance between route and scenario
        )
    ...
    def _create_test_criteria(self):
        ...
        # 'Normal' criteria
        criteria.add_child(
            OutsideRouteLanesTest(self.ego_vehicles[0], route=self.route)
        )
        ...
        criteria.add_child(
            MinimumSpeedRouteTest(
                self.ego_vehicles[0], self.route, checkpoints=4, name="MinSpeedTest"
            )
        )
    ...
    def _create_weather_behavior(self):
        ...
        return RouteWeatherBehavior(
            self.ego_vehicles[0], self.route, self.config.weather
        )
    ...
    def _create_criterion_tree(self, scenario, criteria):
        ...
        criteria_tree.add_child(
            WaitForBlackboardVariable(var_name, True, False, name=check_name)
        )
        ...
        scenario_criteria.add_child(
            WaitForBlackboardVariable(var_name, False, None, name=check_name)
        )

        criteria_tree.add_child(scenario_criteria)
        criteria_tree.add_child(
            Idle()
        )  # Avoid the indiviual criteria stopping the simulation
    ...
    def __del__(self):
        ...
        self.client.apply_batch(
            [carla.command.DestroyActor(x) for x in self._parked_ids]
        )
