...
class EnterActorFlow(BasicScenario):
    ...
    def __init__(
        ...
        self._reference_waypoint = CarlaDataProvider.get_map().get_waypoint(
            ego_location
        )

        self._sink_distance = 2

        self._start_actor_flow = convert_dict_to_location(
            config.other_parameters["start_actor_flow"]
        )
        self._end_actor_flow = convert_dict_to_location(
            config.other_parameters["end_actor_flow"]
        )

        self._flow_speed = get_value_parameter(config, "flow_speed", float, 10)
        self._source_dist_interval = get_interval_parameter(
            config, "source_dist_interval", float, [20, 50]
        )
    ...
    def _create_behavior(self):
        ...
        if self.route_mode:
            ...
            route = grp.trace_route(
                source_wp.transform.location, sink_wp.transform.location
            )
            extra_space = 20
            for i in range(-2, -len(route) - 1, -1):
                current_wp = route[i][0]
                extra_space += current_wp.transform.location.distance(
...
class EnterActorFlowV2(EnterActorFlow):
    ...
    def _create_behavior(self):
        ...
        exit_wp = generate_target_waypoint_in_route(
            self._reference_waypoint, self.config.route
        )
        ...
        if self.route_mode:
            ...
            for i in range(-2, -len(route) - 1, -1):
                current_wp = route[i][0]
                self._extra_space += current_wp.transform.location.distance(
                    route[i + 1][0].transform.location
                )
                if current_wp.is_junction:
                    break
...
class InterurbanActorFlow(BasicScenario):
    ...
    def __init__(
        ...
        route_entry_wp, route_exit_wp = self._get_entry_exit_route_lanes(
            self._reference_wp, config.route
        )
        route_exit_wp = route_exit_wp.next(8)[
            0
        ]  # Just in case the junction maneuvers don't match
    ...
    def _get_entry_exit_route_lanes(self, wp, route):
        ...
        for i in range(index, len(route)):
            ...
            if reached_junction and (
                road_option
                not in (RoadOption.LEFT, RoadOption.RIGHT, RoadOption.STRAIGHT)
            ):
...
class InterurbanAdvancedActorFlow(BasicScenario):
    ...
    def __init__(
        ...
        self._reference_wp = self._map.get_waypoint(config.trigger_points[0].location)
        self._exit_wp = generate_target_waypoint_in_route(
            self._reference_wp, config.route
        )

        self._start_actor_flow_1 = convert_dict_to_location(
            config.other_parameters["start_actor_flow"]
        )
        self._end_actor_flow_1 = convert_dict_to_location(
            config.other_parameters["end_actor_flow"]
        )
    ...
    def _initialize_actors(self, config):
        ...
        if (
            not self._source_wp_2
            or self._source_wp_2.lane_type != carla.LaneType.Driving
        ):
        ...
        if self.route_mode:
            grp = CarlaDataProvider.get_global_route_planner()
            route = grp.trace_route(
                self._source_wp_2.transform.location, self._sink_wp_2.transform.location
            )
            self._extra_space = 20
            route_exit_wp = None
            for i in range(-2, -len(route) - 1, -1):
    ...
    def _create_behavior(self):
        ...
        root.add_child(
            WaitUntilInFrontPosition(self.ego_vehicles[0], self._sink_wp_2.transform)
        )
        root.add_child(
            ActorFlow(
                self._source_wp_1,
