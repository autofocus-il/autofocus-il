...
class StationaryObjectCrossing(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        location, _ = get_location_in_distance_from_wp(
            self._reference_waypoint, _distance
        )
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Parallel(
            name="StaticObstacle", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE
        )
...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(
        ...
        self._reaction_time = (
            2.1  # Time the agent has to react to avoid the collision [s]
        )
        ...
        self._blocker_model = get_value_parameter(
            config, "blocker_model", str, "static.prop.vendingmachine"
        )
        if abs(self._crossing_angle) > 90:
            raise ValueError(
                "'crossing_angle' must be between -90 and 90º for the pedestrian to cross the road"
            )
        self._direction = get_value_parameter(config, "direction", str, "right")
        if self._direction not in ("left", "right"):
            raise ValueError(
                f"'direction' must be either 'right' or 'left' but {self._direction} was given"
            )
    ...
    def _get_sidewalk_transform(self, waypoint, offset):
        ...
        if waypoint.lane_type == carla.LaneType.Sidewalk:
            ...
            offset_location = carla.Location(
                offset_dist * right_vector.x, offset_dist * right_vector.y
            )
    ...
    def _initialize_actors(self, config):
        ...
        while self._number_of_attempts > 0:
            ...
            offset = {
                "yaw": 0 if "vehicle" in self._blocker_model else 90,
                "z": 0.0,
                "k": 1.5,
            }
            self._blocker_transform = self._get_sidewalk_transform(
                sidewalk_waypoint, offset
            )
            ...
            adversary = CarlaDataProvider.request_new_actor(
                "walker.*", self._adversary_transform
            )
            ...
            self._collision_dist += waypoint.transform.location.distance(
        ...
        if self._number_of_attempts == 0:
            raise Exception(
                "Couldn't find viable position for the adversary and blocker actors"
            )

        blocker.set_simulate_physics(False)
        adversary.set_location(
            self._adversary_transform.location + carla.Location(z=-200)
        )
    ...
    def _create_behavior(self):
        ...
        sequence.add_child(
            ActorTransformSetter(self.other_actors[0], self._adversary_transform, True)
        )
        ...
        # Move the adversary
        move_distance = (
            2 * self._collision_dist
        )  # Cross the whole road (supposing symetry in both directions)
...
class ParkingCrossingPedestrian(BasicScenario):
    ...
    def _get_blocker_transform(self, waypoint):
        ...
        if waypoint.lane_type == carla.LaneType.Sidewalk:
            ...
            offset_location = carla.Location(
                waypoint.lane_width * vector.x, waypoint.lane_width * vector.y
            )
            new_location = waypoint.transform.location + offset_location
        new_location.z += 0.5
    ...
    def _get_walker_transform(self, waypoint):
        ...
        new_rotation = waypoint.transform.rotation
        new_rotation.yaw += (
            270 - self._crossing_angle
            if self._direction == "right"
            else 90 + self._crossing_angle
        )
    ...
    def _create_behavior(self):
        ...
        sequence.add_child(
            ActorTransformSetter(self.other_actors[1], self._walker_transform, True)
        )
        collision_location = self._collision_wp.transform.location
        ...
        # Move the adversary
        distance = (
            8.0  # Scenario is meant to be used at a one lane - one direction road
        )
    ...
    def _setup_scenario_trigger(self, config):
        ...
        parallel.add_child(
            MovePedestrianWithEgo(self.ego_vehicles[0], self.other_actors[1], 100)
        )
