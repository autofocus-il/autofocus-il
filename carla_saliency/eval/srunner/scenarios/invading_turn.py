...
class InvadingTurn(BasicScenario):
    ...
    def __init__(
        ...
        self._offset = get_value_parameter(
            config, "offset", float, 0.25
        )  # meters invaded in the opposite direction
    ...
    def _initialize_actors(self, config):
        ...
        self._true_offset = (
            self._offset + self._sink_wp.lane_width / 2 - self._offset_constant
        )
    ...
    def _create_obstacle(self):
        ...
        while dist < obstacle_distance:
            ...
            spawn_transform.location += carla.Location(
                x=displacement * r_vec.x, y=displacement * r_vec.y, z=0.3
            )

            cone = CarlaDataProvider.request_new_actor(
                "*constructioncone*", spawn_transform
            )
