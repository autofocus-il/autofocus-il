...
class ParkingExit(BasicScenario):
    ...
    def __init__(
        ...
        self._front_vehicle_distance = get_value_parameter(
            config, "front_vehicle_distance", float, 20
        )
        self._behind_vehicle_distance = get_value_parameter(
            config, "behind_vehicle_distance", float, 10
        )
    ...
    def _initialize_actors(self, config):
        ...
        self._ego_location = self._get_displaced_location(
            self.ego_vehicles[0], self._parking_waypoint
        )
    ...
    def _create_behavior(self):
        ...
        side_actor_behavior.add_child(
            DriveDistance(self.other_actors[2], self._side_end_distance)
        )
        side_actor_behavior.add_child(
            ActorTransformSetter(self.other_actors[2], self._end_side_transform, False)
        )
