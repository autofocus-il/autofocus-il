...
class CrossingBicycleFlow(BasicScenario):
    ...
    def __init__(
        ...
        self._start_flow = convert_dict_to_location(
    ...
    def _initialize_actors(self, config):
        ...
        # Get the plan
        self._source_wp = self._map.get_waypoint(
            self._start_flow, lane_type=carla.LaneType.Biking
        )
        if (
            not self._source_wp
            or self._source_wp.transform.location.distance(self._start_flow) > 10
        ):
        ...
        while True:
            ...
            if plan_step == 0 and wp.is_junction:
                ...
                plan_step == 2
                and exit_loc.distance(wp.transform.location) > self._end_dist_flow
            ):
                break
    ...
    def _create_behavior(self):
        ...
        if self._signalized_junction:
            ...
            tl_freezer_sequence.add_child(
                TrafficLightFreezer(
                    self._init_tl_dict, duration=self._green_light_delay
                )
            )
