...
class StaticCutIn(BasicScenario):
    ...
    def __init__(
        ...
        self._reaction_time = (
            2.7  # Time the agent has to react to avoid the collision [s]
        )
    ...
    def _initialize_actors(self, config):
        ...
        for i in range(self._back_vehicles):
            # Move to the side
            side_wp = (
                blocker_wp.get_left_lane()
                if self._direction == "left"
                else blocker_wp.get_right_lane()
            )
            if not side_wp:
                for actor in self.other_actors:
                    actor.destroy()
                raise ValueError(
                    "Couldn't find a proper position for the cut in vehicle"
                )
            ...
            blocker_actor = CarlaDataProvider.request_new_actor(
                ...
                side_wp.transform,
                "scenario",
                attribute_filter=self._attributes,
            )
            ...
            blocker_actor.set_location(
                side_wp.transform.location + carla.Location(z=-500)
            )
        ...
        self._adversary_actor.set_location(
            side_wp.transform.location + carla.Location(z=-500)
        )
        ...
        self._adversary_actor.apply_control(
    ...
    def _create_behavior(self):
        ...
        if self.route_mode:
            ...
            total_dist += self._vehicle_gap * (
                self._back_vehicles + self._front_vehicles + 1
            )
