...
class ConstructionObstacle(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        if not wps:
            raise ValueError(
                "Couldn't find a viable position to set up the construction actors"
            )
        self._construction_wp = wps[0]
        self._create_construction_setup(
            self._construction_wp.transform, self._reference_waypoint.lane_width
        )

        self._end_wp = self._move_waypoint_forward(
            self._construction_wp, self._end_distance
        )
    ...
    def _spawn_side_prop(self, wp):
        ...
        signal_prop = CarlaDataProvider.request_new_actor(
            "static.prop.warningconstruction", spawn_transform
        )
        if not signal_prop:
    ...
    def _create_cones_side(
        ...
        while _dist < (cone_length * cone_offset):
            ...
            cone = CarlaDataProvider.request_new_actor(
                "static.prop.constructioncone", spawn_transform
            )
    ...
    def _create_behavior(self):
        ...
        behavior.add_child(
            InTriggerDistanceToLocation(
                self.ego_vehicles[0],
                self._construction_wp.transform.location,
