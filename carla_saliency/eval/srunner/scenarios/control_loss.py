...
class ControlLoss(BasicScenario):
    ...
    def _initialize_actors(self, config):
        ...
        first_wp, _ = get_waypoint_in_distance(
            self._reference_waypoint, self._distance[0]
        )
        first_ground_loc = self.world.ground_projection(
            first_wp.transform.location + carla.Location(z=1), 2
        )
        first_loc = (
            first_ground_loc.location
            if first_ground_loc
            else first_wp.transform.location
        )
        self.first_transform = carla.Transform(first_loc, first_wp.transform.rotation)

        second_wp, _ = get_waypoint_in_distance(
            self._reference_waypoint, self._distance[1]
        )
        second_ground_loc = self.world.ground_projection(
            second_wp.transform.location + carla.Location(z=1), 2
        )
        second_loc = (
            second_ground_loc.location
            if second_ground_loc
            else second_wp.transform.location
        )
        self.second_transform = carla.Transform(
            second_loc, second_wp.transform.rotation
        )

        third_wp, _ = get_waypoint_in_distance(
            self._reference_waypoint, self._distance[2]
        )
        third_ground_loc = self.world.ground_projection(
            third_wp.transform.location + carla.Location(z=1), 2
        )
        third_loc = (
            third_ground_loc.location
            if third_ground_loc
            else third_wp.transform.location
        )
    ...
    def _create_behavior(self):
        ...
        root = py_trees.composites.Parallel(
            "ControlLoss", py_trees.common.ParallelPolicy.SUCCESS_ON_ONE
        )
        ...
        noise_behavior_1 = py_trees.composites.Parallel(
            "Add Noise 1", py_trees.common.ParallelPolicy.SUCCESS_ON_ONE
        )
        noise_behavior_1.add_child(AddNoiseToRouteEgo(self.ego_vehicles[0], *noise_1))
        noise_behavior_1.add_child(
            InTriggerDistanceToLocation(
                self.ego_vehicles[0],
                self.first_transform.location,
                self._trigger_dist,
                operator.gt,
            )
        )
        ...
        noise_behavior_2 = py_trees.composites.Parallel(
            "Add Noise 2", py_trees.common.ParallelPolicy.SUCCESS_ON_ONE
        )
        noise_behavior_2.add_child(AddNoiseToRouteEgo(self.ego_vehicles[0], *noise_2))
        noise_behavior_2.add_child(
            InTriggerDistanceToLocation(
                self.ego_vehicles[0],
                self.second_transform.location,
                self._trigger_dist,
                operator.gt,
            )
        )
        ...
        noise_behavior_3 = py_trees.composites.Parallel(
            "Add Noise 3", py_trees.common.ParallelPolicy.SUCCESS_ON_ONE
        )
        noise_behavior_3.add_child(AddNoiseToRouteEgo(self.ego_vehicles[0], *noise_3))
        noise_behavior_3.add_child(
            InTriggerDistanceToLocation(
                self.ego_vehicles[0],
                self.third_transform.location,
                self._trigger_dist,
                operator.gt,
            )
        )
