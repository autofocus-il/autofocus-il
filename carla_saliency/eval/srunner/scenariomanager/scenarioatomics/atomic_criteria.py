...
class Criterion(py_trees.behaviour.Behaviour):
    ...
    def __init__(self, name, actor, optional=False, terminate_on_failure=False):
        ...
        self.test_status = (
            "INIT"  # Either "INIT", "RUNNING", "SUCCESS", "ACCEPTABLE" or "FAILURE"
        )
...
class MaxVelocityTest(Criterion):
    ...
    def __init__(
        self, actor, max_velocity, optional=False, name="CheckMaximumVelocity"
    ):
        """
...
class DrivenDistanceTest(Criterion):
    ...
    def update(self):
        ...
        if self.actual_value > self.success_value:
            ...
            and self.actual_value > self.acceptable_value
        ):
...
class CollisionTest(Criterion):
    ...
    MAX_ID_TIME = (
        5  # Two collisions with the same id that happen within this time count as one
    )
    EPSILON = (
        0.1  # Collisions at lower this speed won't be counted as the actor's fault
    )
    ...
    def _count_collisions(self, event):  # pylint: disable=too-many-return-statements
        ...
        collision_event = TrafficEvent(
            event_type=actor_type, frame=GameTime.get_frame()
        )
        collision_event.set_dict(
            {"other_actor": event.other_actor, "location": actor_location}
        )
    ...
    def update(self):
        ...
        if active is not None:
            ...
            py_trees.blackboard.Blackboard().set(
                "AC_SwitchActorBlockedTest", None, overwrite=True
            )
...
class OnSidewalkTest(Criterion):
    ...
    def __init__(
        ...
        super(OnSidewalkTest, self).__init__(
            name, actor, optional, terminate_on_failure
        )
    ...
    def update(self):
        ...
        distance_vector = (
            CarlaDataProvider.get_location(self.actor) - self._actor_location
        )
        distance = math.sqrt(
...
class OutsideRouteLanesTest(Criterion):
    ...
    ALLOWED_OUT_DISTANCE = (
        0.5  # At least 0.5, due to the mini-shoulder between lanes and sidewalks
    )
    ...
    def update(self):
        ...
        if active is not None:
            ...
            py_trees.blackboard.Blackboard().set(
                "AC_SwitchWrongDirectionTest", None, overwrite=True
            )

        self._is_outside_driving_lanes(location)
        self._is_at_wrong_lane(location)

        if self._outside_lane_active or (
            self._wrong_direction_active and self._wrong_lane_active
        ):
            self.test_status = "FAILURE"
        ...
        for index in range(
            ...
            if wp_veh.dot(wp_dir) > 0:
                ...
                prev_route_location = self._route_transforms[
                    self._current_index
                ].location
    ...
    def _is_at_wrong_lane(self, location):
        ...
        if waypoint.is_junction:
            ...
            if self._last_ego_waypoint.is_junction:
                ...
                if angle < self.MAX_VEHICLE_ANGLE or angle > (
                    360 - self.MAX_VEHICLE_ANGLE
                ):
                    self._wrong_lane_active = False
                else:
                ...
                if angle > self.MAX_WAYPOINT_ANGLE and angle < (
                    360 - self.MAX_WAYPOINT_ANGLE
                ):
...
class InRouteTest(Criterion):
    ...
    def __init__(
        ...
        super(InRouteTest, self).__init__(
            name, actor, terminate_on_failure=terminate_on_failure
        )
        self.units = None  # We care about whether or not it fails, no units attached
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            for index in range(
                ...
                distance = math.sqrt(
                    ((location.x - ref_location.x) ** 2)
                    + ((location.y - ref_location.y) ** 2)
                )
...
class RouteCompletionTest(Criterion):
    ...
    def __init__(
        ...
        self._traffic_event = TrafficEvent(
            event_type=TrafficEventType.ROUTE_COMPLETION, frame=0
        )
        self._traffic_event.set_dict({"route_completed": self.actual_value})
        self._traffic_event.set_message(
            "Agent has completed {} of the route".format(self.actual_value)
        )
        self.events.append(self._traffic_event)
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            for index in range(
                self._index,
                min(self._index + self.WINDOWS_SIZE + 1, self._route_length),
            ):
                # Get the dot product to know if it has passed this location
                route_transform = self._route_transforms[index]
                route_location = route_transform.location
                wp_dir = (
                    route_transform.get_forward_vector()
...
class RunningRedLightTest(Criterion):
    ...
    def update(self):
        ...
        tail_close_pt = self.rotate_point(
            carla.Vector3D(-0.8 * veh_extent, 0, 0), transform.rotation.yaw
        )
        tail_close_pt = location + carla.Location(tail_close_pt)

        tail_far_pt = self.rotate_point(
            carla.Vector3D(-veh_extent - 1, 0, 0), transform.rotation.yaw
        )
            ...
            for wp in waypoints:
                ...
                if (
                    ...
                    lft_lane_wp = self.rotate_point(
                        carla.Vector3D(0.6 * lane_width, 0, 0), yaw_wp + 90
                    )
                    ...
                    if self.is_vehicle_crossing_line(
                        ...
                        red_light_event.set_dict(
                            {"id": traffic_light.id, "location": location}
                        )
...
class RunningStopTest(Criterion):
    ...
    def update(self):
        ...
        if not self.is_actor_affected_by_stop(check_wps, self._target_stop_sign):
            if (
                not self._stop_completed
                and self._last_failed_stop != self._target_stop_sign.id
            ):
                # did we stop?
                self.actual_value += 1
                self.test_status = "FAILURE"
                ...
                running_stop_event.set_dict(
                    {"id": self._target_stop_sign.id, "location": stop_location}
                )
...
class MinimumSpeedRouteTest(Criterion):
    ...
    def update(self):
        ...
        for index in range(
            self._index, min(self._index + self.WINDOWS_SIZE + 1, self._route_length)
        ):
            # Get the dot product to know if it has passed this location
        ...
        background_vehicles = [
            v for v in all_vehicles if v.attributes["role_name"] == "background"
        ]
    ...
    def _set_traffic_event(self):
        ...
        if self._speed_points > 0 and self._mean_speed:
            ...
            checkpoint_value = round(
                self._actor_speed / (self.RATIO * self._mean_speed) * 100, 2
            )
        else:
        ...
        if checkpoint_value >= self.success_value:
            ...
            self._traffic_event = TrafficEvent(
                TrafficEventType.MIN_SPEED_INFRACTION, GameTime.get_frame()
            )
    ...
    def terminate(self, new_status):
        ...
        if len(self._checkpoint_values):
            self.actual_value = round(
                sum(self._checkpoint_values) / len(self._checkpoint_values), 2
            )
        ...
        if not self._terminated:
            if self.test_status == "FAILURE":
                traffic_event = TrafficEvent(
                    TrafficEventType.YIELD_TO_EMERGENCY_VEHICLE, GameTime.get_frame()
                )
                traffic_event.set_message(
                    "Agent failed to yield to an emergency vehicle"
                )
...
class ScenarioTimeoutTest(Criterion):
    ...
    def __init__(
        self, actor, scenario_name, optional=False, name="ScenarioTimeoutTest"
    ):
        """
        Constructor
        """
    ...
    def terminate(self, new_status):
        ...
        if timeout:
            ...
            traffic_event = TrafficEvent(
                event_type=TrafficEventType.SCENARIO_TIMEOUT, frame=GameTime.get_frame()
            )
