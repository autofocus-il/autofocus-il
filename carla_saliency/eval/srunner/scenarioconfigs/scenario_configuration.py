...
class ActorConfigurationData(object):
    ...
    def parse_from_node(node, rolename):
        ...
        transform = carla.Transform(
            carla.Location(x=pos_x, y=pos_y, z=pos_z), carla.Rotation(yaw=yaw)
        )
        ...
        return ActorConfigurationData(
            model, transform, rolename, speed, autopilot, random_location, color
        )
    ...
    def parse_from_dict(actor_dict, rolename):
        ...
        random_location = (
            actor_dict["random_location"] if "random_location" in actor_dict else False
        )
